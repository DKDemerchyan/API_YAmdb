...
class Test06CommentAPI:
    ...
    def test_01_comment_not_auth(self, client, admin_client, admin):
        ...
        response = client.get(f'/api/v1/titles/{titles[0]["id"]}/review/{reviews[0]["id"]}/comments/')
        assert response.status_code != 404, (
            'Страница `/api/v1/titles/{title_id}/review/{review_id}/comments/` '
            'не найдена, проверьте этот адрес в *urls.py*'
        )
        assert response.status_code == 200, (
            'Проверьте, что при GET запросе `/api/v1/titles/{title_id}/review/{review_id}/comments/` '
            'без токена авторизации возвращается статус 200'
        )

    def create_comment(self, client_user, title_id, review_id, text):
        data = {'text': text}
        response = client_user.post(f'/api/v1/titles/{title_id}/review/{review_id}/comments/', data=data)
        assert response.status_code == 201, (
            'Проверьте, что при POST запросе `/api/v1/titles/{title_id}/review/{review_id}/comments/` '
    ...
    def test_02_comment(self, admin_client, admin):
        ...
        response = admin_client.post(
            f'/api/v1/titles/{titles[0]["id"]}/review/{reviews[0]["id"]}/comments/', data=data
        )
        ...
        data = {'text': 'kjdfg'}
        response = admin_client.post('/api/v1/titles/999/review/999/comments/', data=data)
        assert response.status_code == 404, (
            'Проверьте, что при POST запросе `/api/v1/titles/{title_id}/review/{review_id}/comments/` '
        ...
        response = admin_client.get(f'/api/v1/titles/{titles[0]["id"]}/review/{reviews[0]["id"]}/comments/')
        assert response.status_code == 200, (
            'Проверьте, что при GET запросе `/api/v1/titles/{title_id}/review/{review_id}/comments/` '
    ...
    def test_03_review_detail(self, client, admin_client, admin):
        ...
        pre_url = f'/api/v1/titles/{titles[0]["id"]}/review/{reviews[0]["id"]}/comments/'
        response = client.get(f'{pre_url}{comments[0]["id"]}/')
        assert response.status_code != 404, (
            'Страница `/api/v1/titles/{title_id}/review/{review_id}/comments/{comment_id}/` '
            'не найдена, проверьте этот адрес в *urls.py*'
        )
        assert response.status_code == 200, (
            'Проверьте, что при GET запросе `/api/v1/titles/{title_id}/review/{review_id}/comments/{comment_id}/` '
            'без токена авторизации возвращается статус 200'
        )
        data = response.json()
        ...
        assert response.status_code == 200, (
            'Проверьте, что при PATCH запросе `/api/v1/titles/{title_id}/review/{review_id}/comments/{comment_id}/` '
        ...
        assert response.status_code == 204, (
            'Проверьте, что при DELETE запросе `/api/v1/titles/{title_id}/review/{review_id}/comments/{comment_id}/` '
    ...
    def test_04_comment_check_permission(self, client, admin_client, admin):
        ...
        assert response.status_code == 401, (
            ...
            '`/api/v1/titles/{{title_id}}/review/{{review_id}}/comments/{{comment_id}}/` '
            'без токена авторизации возвращается статус 401'
        )
