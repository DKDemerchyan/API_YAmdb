...
class Test05ReviewAPI:
    ...
    def test_01_review_not_auth(self, client, admin_client):
        ...
        assert response.status_code != 404, (
            'Страница `/api/v1/titles/{title_id}/review/` не найдена, проверьте этот адрес в *urls.py*'
        )
        assert response.status_code == 200, (
            'Проверьте, что при GET запросе `/api/v1/titles/{title_id}/review/` '
    ...
    def create_review(self, client_user, title_id, text, score):
        ...
        response = client_user.post(f'/api/v1/titles/{title_id}/review/', data=data)
        assert response.status_code == 201, (
            'Проверьте, что при POST запросе `/api/v1/titles/{title_id}/review/` '
    ...
    def test_02_review_admin(self, admin_client, admin):
        ...
        data = {}
        response = admin_client.post(f'/api/v1/titles/{titles[0]["id"]}/review/', data=data)
        ...
        assert response.status_code == code, (
            'Проверьте, что при POST запросе на `/api/v1/titles/{title_id}/review/` '
        ...
        response = admin_client.put(f'/api/v1/titles/{titles[0]["id"]}/review/', data=data)
        code = 405
        assert response.status_code == code, (
            'Проверьте, что PUT запрос на `/api/v1/titles/{title_id}/review/` '
        ...
        response = admin_client.post('/api/v1/titles/999/review/', data=data)
        assert response.status_code == 404, (
            'Проверьте, что при POST запросе `/api/v1/titles/{title_id}/review/` '
        ...
        response = admin_client.get(f'/api/v1/titles/{titles[0]["id"]}/review/')
        assert response.status_code == 200, (
            'Проверьте, что при GET запросе `/api/v1/titles/{title_id}/review/` возвращает статус 200'
        )
        data = response.json()
        assert 'count' in data, (
            'Проверьте, что при GET запросе `/api/v1/titles/{title_id}/review/` возвращаете данные с пагинацией. '
    ...
    def test_03_review_detail(self, client, admin_client, admin):
        ...
        response = client.get(f'/api/v1/titles/{titles[0]["id"]}/review/{reviews[0]["id"]}/')
        assert response.status_code != 404, (
            'Страница `/api/v1/titles/{title_id}/review/{review_id}/` не найдена, проверьте этот адрес в *urls.py*'
        )
        assert response.status_code == 200, (
            'Проверьте, что при GET запросе `/api/v1/titles/{title_id}/review/{review_id}/` '
            'без токена авторизации возвращается статус 200'
        )
        data = response.json()
        ...
        response = admin_client.patch(f'/api/v1/titles/{titles[0]["id"]}/review/{reviews[0]["id"]}/', data=data)
        assert response.status_code == 200, (
            'Проверьте, что при PATCH запросе `/api/v1/titles/{title_id}/review/{review_id}/` '
        ...
        response = admin_client.get(f'/api/v1/titles/{titles[0]["id"]}/review/{reviews[0]["id"]}/')
        assert response.status_code == 200, (
            'Проверьте, что при GET запросе `/api/v1/titles/{title_id}/review/{review_id}/` '
            'без токена авторизации возвращается статус 200'
        )
        ...
        response = client_user.patch(f'/api/v1/titles/{titles[0]["id"]}/review/{reviews[2]["id"]}/', data=data)
        ...
        response = client_user.patch(f'/api/v1/titles/{titles[0]["id"]}/review/{reviews[1]["id"]}/', data=data)
        assert response.status_code == 200, (
            'Проверьте, что при PATCH запросе `/api/v1/titles/{title_id}/review/{review_id}/` '
            'возвращается статус 200'
        )
        ...
        client_moderator = auth_client(moderator)
        response = client_moderator.delete(f'/api/v1/titles/{titles[0]["id"]}/review/{reviews[1]["id"]}/')
        assert response.status_code == 204, (
            'Проверьте, что при DELETE запросе `/api/v1/titles/{title_id}/review/{review_id}/` '
            'возвращаете статус 204'
        )
        response = admin_client.get(f'/api/v1/titles/{titles[0]["id"]}/review/')
        test_data = response.json()['results']
        assert len(test_data) == len(reviews) - 1, (
            'Проверьте, что при DELETE запросе `/api/v1/titles/{title_id}/review/{review_id}/` удаляете объект'
        )

    def check_permissions(self, user, user_name, reviews, titles):
        client_user = auth_client(user)
        data = {'text': 'jdfk', 'score': 7}
        response = client_user.patch(f'/api/v1/titles/{titles[0]["id"]}/review/{reviews[0]["id"]}/', data=data)
        assert response.status_code == 403, (
            f'Проверьте, что при PATCH запросе `/api/v1/titles/{{title_id}}/review/{{review_id}}/` '
            f'с токеном авторизации {user_name} возвращается статус 403'
        )
        response = client_user.delete(f'/api/v1/titles/{titles[0]["id"]}/review/{reviews[0]["id"]}/')
        assert response.status_code == 403, (
            f'Проверьте, что при DELETE запросе `/api/v1/titles/{{title_id}}/review/{{review_id}}/` '
            f'с токеном авторизации {user_name} возвращается статус 403'
        )
    ...
    def test_04_reviews_check_permission(self, client, admin_client, admin):
        reviews, titles, user, moderator = create_reviews(admin_client, admin)
        data = {'text': 'jdfk', 'score': 7}
        response = client.post(f'/api/v1/titles/{titles[0]["id"]}/review/', data=data)
        assert response.status_code == 401, (
            'Проверьте, что при POST запросе `/api/v1/titles/{{title_id}}/review/` '
            'без токена авторизации возвращается статус 401'
        )
        response = client.patch(f'/api/v1/titles/{titles[0]["id"]}/review/{reviews[1]["id"]}/', data=data)
        assert response.status_code == 401, (
            'Проверьте, что при PATCH запросе `/api/v1/titles/{{title_id}}/review/{{review_id}}/` '
            'без токена авторизации возвращается статус 401'
        )
        response = client.delete(f'/api/v1/titles/{titles[0]["id"]}/review/{reviews[1]["id"]}/')
        assert response.status_code == 401, (
            'Проверьте, что при DELETE запросе `/api/v1/titles/{{title_id}}/review/{{review_id}}/` '
            'без токена авторизации возвращается статус 401'
        )
